...
describe('listIsraeliHolidaysForYear', () => {
  it('maps 2026 Hebrew holidays to the civil dates Israel observes', () => {
    ...
    expect(byCode.pesach).toEqual({ code: 'pesach', nameHe: 'פסח', date: '2026-04-01' });
    expect(byCode.pesach_7).toEqual({ code: 'pesach_7', nameHe: 'שביעי של פסח', date: '2026-04-07' });
    expect(byCode.yom_haatzmaut).toEqual({
      code: 'yom_haatzmaut',
      nameHe: 'יום העצמאות',
      date: '2026-04-21',
    });
    expect(byCode.shavuot).toEqual({ code: 'shavuot', nameHe: 'שבועות', date: '2026-05-21' });
    expect(byCode.rosh_hashana_1?.date).toBe('2026-09-11');
    expect(byCode.rosh_hashana_2?.date).toBe('2026-09-12');
    expect(byCode.yom_kippur?.date).toBe('2026-09-20');
    expect(byCode.sukkot?.date).toBe('2026-09-25');
    expect(byCode.simchat_torah?.date).toBe('2026-10-02');
  ...
  it('puts Independence Day on a weekday Israel observes, not Friday or Saturday', () => {
    ...
    expect(atzmaut?.date).toBe('2026-04-21');
