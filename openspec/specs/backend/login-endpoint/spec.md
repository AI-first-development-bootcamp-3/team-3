# backend/login-endpoint Specification
...
Requests SHALL be subject to the throttling defined by `backend/login-rate-limiting`.
...
- **WHEN** a request to `PATCH /me/password` arrives from a caller who has exceeded a configured attempt threshold within the window
- **THEN** the service responds `429` using the standard error contract
