...
**Attachment reconciliation on update: `attachmentIds` is the full desired set, not a diff.** On `updateAbsence`, attachments currently linked to this absence but absent from the submitted `attachmentIds` are unlinked (`absenceId` set back to `null`); ids present in `attachmentIds` but not yet linked are linked, same ownership constraint (`uploaderId = userId`) as `createAbsence` uses today. This keeps the contract simple: the client always submits "here is the complete set of files that should be attached now," matching how the form already tracks `uploadedFiles` as one list in state.
